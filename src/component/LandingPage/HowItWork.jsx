...
export default function HowItWorks() {
  return (
    <section className= "bg-[linear-gradient(360deg,#2e3a53_0%,#2e3a53_100%)] py-15 mt-25 px-2 text-center" >
      <span className="inline-block bg-blue-800 text-white text-xs font-semibold tracking-widest uppercase px-4 py-1.5 rounded-full mb-4">
        Process
      </span>
      ...
      <div className="relative max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10">
        ...
        <div className="hidden md:block absolute top-9 left-[12.5%] right-[12.5%] h-px bg-gradient-to-r from-blue-600 via-blue-300 to-blue-600 z-0" />

        {steps.map(({ number, title, desc }) => (
          <div key={number} className="relative z-10 flex flex-col items-center text-center px-2">
            <div className="w-[72px] h-[72px] rounded-full bg-gray-800 border-2 border-blue-700 flex items-center justify-center text-2xl font-bold text-white mb-5">
              {number}
            </div>
